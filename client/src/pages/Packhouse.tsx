...
export default function Packhouse() {
  ...
  return (
    ...
    >
      ...
      <div className="px-6 py-6 border-b border-slate-200 bg-white shrink-0">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setShowColdStorage(!showColdStorage)} className={`flex items-center gap-1.5 px-3 py-1.5 text-[13px] font-medium rounded-md transition-colors shadow-sm ${showColdStorage ? 'text-white bg-blue-600 hover:bg-blue-700' : 'text-slate-700 bg-white border border-slate-300 hover:bg-slate-50'}`}>
              <Thermometer size={14} />
              {showColdStorage ? 'Show Kanban' : 'Cold Storage Logs'}
            </button>
            {user?.role === 'Admin' && (
      ...
      {/* Kanban Board Area OR Cold Storage Logs */}
      {showColdStorage ? (
        <div className="flex-1 overflow-auto p-6">
          <div className="bg-white rounded-lg border border-slate-200 shadow-sm">
            <div className="px-5 py-3.5 border-b border-slate-200 flex items-center justify-between">
              <h3 className="text-[14px] font-semibold text-slate-900">Cold Storage Temperature Logs</h3>
              <span className="px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 text-[10px] font-semibold">{lots.filter(l => l.status === 'Cold Storage' || l.status === 'Storage').length} LOTS</span>
            </div>
            <table className="w-full text-left border-collapse text-[13px]">
              <thead className="bg-slate-50 text-[11px] font-semibold text-slate-500 uppercase tracking-wider border-b border-slate-200">
                <tr>
                  ...
                  <th className="px-5 py-3 text-center">Temp</th>
                  <th className="px-5 py-3 text-center">Status</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {lots.filter(l => l.status === 'Cold Storage' || l.status === 'Storage').length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-5 py-12 text-center text-slate-400">
                      <Thermometer className="mx-auto mb-2 text-slate-300" size={24} />
                      No lots currently in cold storage. Move lots to "Cold Storage" stage in the Kanban board.
                    </td>
                  </tr>
                ) : (
                  lots.filter(l => l.status === 'Cold Storage' || l.status === 'Storage').map(lot => (
                    <tr key={lot.id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-5 py-3"><span className="font-mono text-[12px] font-medium text-slate-600 bg-slate-100 px-2 py-0.5 rounded border border-slate-200">{lot.id}</span></td>
                      <td className="px-5 py-3 text-slate-700 font-medium">{lot.farm_id}</td>
                      <td className="px-5 py-3 text-right font-medium">{lot.weight_tons} T</td>
                      <td className="px-5 py-3 text-center"><span className="px-2 py-0.5 rounded-full text-[11px] font-medium bg-emerald-50 text-emerald-700 border border-emerald-200">{lot.quality_grade || 'Pending'}</span></td>
                      <td className="px-5 py-3 text-center"><span className="text-blue-600 font-semibold">2.4°C</span></td>
                      <td className="px-5 py-3 text-center"><span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-blue-50 text-blue-700 border border-blue-200 uppercase">{lot.status}</span></td>
                    </tr>
                  ))
                )}
              </tbody>
